...
1. **双地图显示系统**
   ...
   - **地图类型标识**: 示例地图和用户地图分别标识，示例地图受保护（不可编辑/删除）

2. **地图编辑系统** 🆕
   - **编辑模式**: 点击用户地图的"编辑"按钮进入编辑模式
   - **地图复制**: 点击"复制"按钮直接创建新地图副本（不进入编辑模式）
   - **编辑现有地图**: 编辑用户地图后保存直接更新原图
   - **新建地图**: 从空白或复制的地图创建新地图，保存时另存为
   - **地图管理**: 
     - 示例地图：只读，可复制，不可编辑/删除
     - 用户地图：可编辑、可删除、可复制
     - 自动生成5位数字ID（如 `map_00001`）
   - **数据持久化**: 使用 localStorage 存储用户地图和编辑数据

3. **像素地图编辑器** 🆕
   - **画布编辑**: 在主画布上直接绘制像素，支持拖拽填充
   - **尺寸设置**: 手动设置画布宽度和高度（10-500像素）
   - **调色板系统**: 
     - 自动解析地图中的颜色并添加到调色板
     - 支持添加自定义颜色（颜色选择器）
     - 实时显示当前选中颜色和RGB值
   - **编辑工具**:
     ...
     - **清空地图**: 一键清空所有像素
     - **保存地图**: 保存到 localStorage 并下载JSON文件
     - **导入图片**: 支持导入图片转换为像素地图（开发中）
   - **编辑模式UI**: 
     - 独立的编辑工具栏
     - 右侧调色板面板
     - 退出编辑按钮
...
10. **数据持久化**
    - **编织进度**: 保存当前进度到 localStorage（按地图分别保存）
    - **加载进度**: 加载上次保存的进度（自动恢复模式状态）
    - **重置进度**: 重置所有进度（清除确认完成状态）
    - **完成确认**: 编织完成且保存后，导航按钮自动禁用
    - **地图数据**: 用户创建和编辑的地图保存在 localStorage
    - **地图列表**: 自动管理示例地图和用户地图列表
...
1. **地图管理**:
   - **选择地图**: 在左侧地图列表中点击选择要编织的地图
   - **复制地图**: 点击地图项的"复制"按钮，直接创建新地图副本
   - **编辑地图**: 点击用户地图的"编辑"按钮进入编辑模式
   - **删除地图**: 点击用户地图的"删除"按钮删除地图（需确认）
   - 地图项显示缩略图、名称、类型标识和颜色统计信息

2. **编辑像素地图** 🆕:
   - **进入编辑模式**: 点击用户地图的"编辑"按钮
   - **绘制像素**: 在画布上点击或拖拽填充像素
   - **选择颜色**: 在右侧调色板中点击选择颜色
   - **添加颜色**: 使用颜色选择器选择颜色，点击"添加到调色板"
   - **调整尺寸**: 在工具栏中设置宽度和高度，点击"应用尺寸"
   - **撤销/重做**: 使用工具栏按钮或快捷键（Ctrl+Z / Ctrl+Y）
   - **保存地图**: 点击"保存地图"按钮
     - 编辑现有地图：直接更新原图
     - 新建地图：显示保存对话框，输入显示名称
   - **退出编辑**: 点击"退出编辑"按钮返回浏览模式
...
- **Canvas API**: 用于网格渲染和交互（主网格、小地图、放大镜、编辑画布）
- **localStorage**: 客户端数据持久化（编织进度、用户地图、地图列表）
- **Bresenham 算法**: 直线路径计算
- **粒子系统**: 动态背景效果
- **性能优化**: 离屏Canvas、事件节流、DOM缓存
- **编辑历史**: 撤销/重做系统（最多50步历史记录）
...
```json
{
  "completedSquares": [{"row": 0, "col": 0}, ...],
  "currentDiagonalIndex": 5,
  "isDiagonalMode": true,
...
- 🆕 **地图管理**: 示例地图和用户地图区分，支持复制、编辑、删除
- 🆕 **智能保存**: 编辑现有地图直接更新，新建地图另存为
- 🆕 **Logo 显示**: 应用主页和应用列表页面显示 PixelKnit Logo
- ✅ 性能优化（离屏Canvas、事件节流、DOM缓存）
- ✅ 完成确认机制
- ✅ 小地图颜色统计
- ✅ 交互工具提示
